...
def pick_top_n_diversity(
		n):  # fully integrated function to generate the top n number of sequences by minimizing normality score within the domain and ranking them. Returns sorted dict least to greatest normality score
...
def comparediversityratings(diversity,
							editdist):  # compares the Levenshteinian diversity ranking to the SVM generated diversity rankings as a percent shared
	iterant = 0
	a = 0
	for diversitycandidateindex in range(len(diversity)):
		for editdistcandidateindex in range(len(editdist)):
			if ((diversity[diversitycandidateindex])[0] == (editdist[editdistcandidateindex])[0]):
				a += 1
	print(str(a / float(str(len(
		diversity)) + '.0') * 100) + "% " + "of sequences in the SVM diversity ranking within the range were shared by the Levenshtein diversity ranking")
